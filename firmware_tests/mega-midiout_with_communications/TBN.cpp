...
//I couldn't make the use of ISR because ki can't pass a memeber function to the ISR callback function
#define USETIPISR false
...
void TBN::loop() {
  ...
  switch (s_current) {
    ...
    case S_LISTENING: {
        ...
        //maybe the ISR is not used, in which case we detect a token here
        if (digitalRead(TIP)) {
          if (ownID != 0) {
            s_current = S_BROADCASTING;
          }
    ...
    //maybe the ISR is not used, in which case we broadcast here
    case S_BROADCASTING: {
        broadcastFunction();
        break;
      }
